...
template <uint32_t C, uint32_t F_C>
    ...
    float* __restrict__ dL_dfeatures)
{
    ...
    __shared__ float collected_features[F_C * BLOCK_SIZE];
	...
	float accum_rec[C] = { 0 };
    float accum_rec_f[F_C] = { 0 };
	float dL_dpixel[C];
    float dL_dpixel_f[F_C];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// const float w = alpha * T; //NOTE: no reference
            ...
            for (int ch = 0; ch < F; ch++)
            {
                ...
                atomicAdd(&(dL_dfeatures[global_id * F + ch]), dchannel_dcolor * dL_dchannel_f);
            }
...
void BACKWARD::render(
    ...
    float* dL_dfeatures)
{
	renderCUDA<NUM_CHANNELS, NUM_F_CHANNELS> << <grid, block >> >(
		ranges,
		point_list,
		F, W, H,
		focal_x, focal_y,
