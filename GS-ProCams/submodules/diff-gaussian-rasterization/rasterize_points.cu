...
//   CHECK_INPUT(campos);
  ...
  torch::Tensor out_features = torch::full({F, H, W}, 0.0, float_opts);
  ...
  return std::make_tuple(rendered, out_color, out_features, out_others, radii, geomBuffer, binningBuffer, imgBuffer);
}
...
//   CHECK_INPUT(geomBuffer);
  ...
  torch::Tensor dL_dfeatures = torch::zeros({P, F}, means3D.options());
  ...
  if(P != 0)
  {  
	  CudaRasterizer::Rasterizer::backward(P, F, degree, M, R,
	  background.contiguous().data<float>(),
	  W, H, 
	  means3D.contiguous().data<float>(),
