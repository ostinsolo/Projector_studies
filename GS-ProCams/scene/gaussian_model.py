...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, pbr=True):
        self.pbr = pbr
        ...
        if self.pbr:
            self._base_color = torch.empty(0) # base color
            self._roughness = torch.empty(0) # roughness
    ...
    def restore(self, model_args, training_args):
        ...
        try: 
            ...
            print("Warning: Not loading optimizer state_dict!")
    ...
    def get_base_color(self):
        assert self.pbr, "This Gaussian model does not have PBR attributes."
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        if self.pbr:
            fused_base_color = torch.tensor(np.asarray(pcd.colors)).float().cuda() 
            roughness = torch.zeros((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda")

            self._base_color = nn.Parameter(fused_base_color.requires_grad_(True))
    ...
    def construct_list_of_attributes(self, save_pbr=False):
        l = ['x', 'y', 'z', 'nx', 'ny', 'nz']
    ...
    def save_ply(self, path, save_pbr=False):
        ...
        attributes_ls = [xyz, normals, f_dc, f_rest, opacities, scale, rotation]
        if self.pbr and save_pbr:
            attributes_ls.extend([
                self._base_color.detach().cpu().numpy(),
                self._roughness.detach().cpu().numpy(),
            ])

        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes(save_pbr)]

        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate(attributes_ls, axis=1)
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        ...
        for group in self.optimizer.param_groups:
            ...
            if stored_state is not None:
                ...
                assert group["params"][0].shape[1:] == extension_tensor.shape[1:], f"{group['name']} {group['params'][0].shape} != {extension_tensor.shape}"
                group["params"][0] = nn.Parameter(torch.cat((group["params"][0], extension_tensor), dim=0).requires_grad_(True))
                optimizable_tensors[group["name"]] = group["params"][0]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation,
        ...
        if self.pbr:
            d.update({"base_color": new_base_color,
